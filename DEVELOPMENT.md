...
- **eslint-config-react-app is dead weight.** It's an unused devDependency (not referenced by `eslint.config.js` or the old pre-migration `.eslintrc.js` either) that generates a wall of stale peer-dependency warnings on every install. Harmless to ignore; safe to remove entirely if it starts causing real friction.
