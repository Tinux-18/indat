...
- **Theming uses `@teispace/next-themes`, not the original `next-themes`.** The original is effectively unmaintained and always renders its anti-flash `<script>` as a literal React-tree child, which both trips a React 19 warning and — since a client-inserted `<script>` never executes — causes a real flash of the wrong theme on every client-side navigation. This matters here because `[locale]` is the outermost layout segment holding `<html>`/`<body>`, so a locale switch remounts the whole tree client-side. The fork injects the script via `useServerInsertedHTML` instead (works correctly on every server-rendered transition, not just the first page load) and supports seeding from a cookie: `app/[locale]/layout.tsx` calls `getTheme()` server-side and passes the result as `initialTheme`, so the theme is correct from the very first byte of every render, locale switches included. See [pacocoursey/next-themes#387](https://github.com/pacocoursey/next-themes/issues/387) for the upstream issue this works around.
